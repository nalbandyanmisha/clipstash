...
fn authorize(
    ...
    let employee = db_connection.find_employee(&employee_name)?;
    let employee_key_code = db_connection.get_keycard(&employee)?;
    let location_access_level = location.required_access_level();
    if employee_key_code.access_level >= location_access_level {
